...
const ClaimFee = ({
  ...
  const [{ isDragging }, drag] = useDrag({
    ...
    item: { id: +claimFee.productId, index },
  });
  ...
  const [, drop] = useDrop(() => ({
    ...
    hover: (draggedItem) => {
      if (draggedItem.index !== index) {
        moveClaimFee(draggedItem.index, index);
  ...
  const handleClickEditClaim = (e, product) => {
    ...
    if (isEditAllowed) {
      ...
      dispatch(actions.handleSetSelectClaimProduct({ ...product, item_type: CLAIM.TYPES.OTHER_FEE }))
  ...
  const claimProgress = claimFee.claim_progress?.[0] || innitProgress;
    ...
    return (
      <tr>
        <td className="productContainerTable__td">
          <div className="productContainerTable__td--text productContainerTable__td--underlineText">
            {isShowDetail ? 'Collapse' : 'View'}
          </div>
        </td>
        <td className="productContainerTable__td productContainerTable__td--space">
          <div className="productContainerTable__td--text">
            $ {formatPriceWithTwoDecimals(+claimFee?.amount)}
          </div>
        </td>
        <td className="productContainerTable__td productContainerTable__td--space">
          <div className="productContainerTable__td--text">
      ...
      >
        ...
        <div className="productContainer__actions productContainer__actions--space">
          <div
            ...
            onClick={(e) => handleClickEditClaim(e, claimFee)}
          >
            <img
              src="/icons/edit.svg"
              alt="edit"
